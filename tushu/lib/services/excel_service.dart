...
class ExcelService {
  ...
  static Future<String?> appendToExisting({
    ...
    // 如果Sheet为空，先写表头；已有表格则按表头顺序追加，避免列错位。
    ...
    if (headers.isEmpty) {
      ...
      if (missingColumns.isNotEmpty) {
        throw StateError("已有Excel缺少字段: ${missingColumns.join(', ')}");
      }
    }
  ...
  static Future<String?> _saveExcel(
    ...
    try {
      final dir = await getApplicationDocumentsDirectory();
      final file = File(overwrite
          ? '${dir.path}/$fileName'
          : await _uniqueFilePath(dir.path, fileName));
  ...
  static Future<String> _uniqueFilePath(String dirPath, String fileName) async {
    ...
    while (await File(candidate).exists()) {
      candidate = '$dirPath/${baseName}_$counter$extension';
      counter += 1;
    }
    return candidate;
  }
