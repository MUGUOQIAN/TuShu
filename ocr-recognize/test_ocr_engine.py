...
class OcrEngineTest(unittest.TestCase):
    ...
    def test_mixed_address_and_phone_line_is_kept(self):
        ...
        self.assertEqual("中山路128号 电话：021-12345678", result["地址"])
    ...
    def test_coal_water_furniture_are_companies_not_names(self):
        coal = ocr_engine._map_business_card_fields(
            [
                "兖州煤业",
                "张伟",
                "项目经理",
        ...
        self.assertEqual("张伟", coal["姓名"])
        self.assertEqual("兖州煤业", coal["公司"])
        self.assertEqual("项目经理", coal["职位"])
        self.assertNotEqual("兖州煤业", coal["姓名"])

        water = ocr_engine._map_business_card_fields(
            [
                "北控水务",
                "王芳",
                "工程师",
        ...
        self.assertEqual("王芳", water["姓名"])
        self.assertEqual("北控水务", water["公司"])
        self.assertEqual("工程师", water["职位"])
        self.assertNotEqual("北控水务", water["姓名"])

        gas = ocr_engine._map_business_card_fields(
            [
                "新奥燃气",
                "陈明",
                "销售经理",
        ...
        self.assertEqual("陈明", gas["姓名"])
        self.assertEqual("新奥燃气", gas["公司"])
        self.assertEqual("销售经理", gas["职位"])
        self.assertNotEqual("新奥燃气", gas["姓名"])

        gas_title = ocr_engine._map_business_card_fields(
            [
                "国家电网",
                "燃气工程师",
                "张伟",
        ...
        self.assertEqual("张伟", gas_title["姓名"])
        self.assertEqual("国家电网", gas_title["公司"])
        self.assertEqual("燃气工程师", gas_title["职位"])
        self.assertNotEqual("燃气", gas_title["姓名"])
        self.assertNotEqual("燃气工程师", gas_title["公司"])

        gas_addr = ocr_engine._map_business_card_fields(
            [
                "新奥燃气",
                "陈明",
                "销售经理",
        ...
        self.assertEqual("陈明", gas_addr["姓名"])
        self.assertEqual("新奥燃气", gas_addr["公司"])
        self.assertEqual("燃气路88号", gas_addr["地址"])
        self.assertNotEqual("燃气路88号", gas_addr["公司"])

        deco = ocr_engine._map_business_card_fields(
            [
                "金螳螂装饰",
                "赵敏",
                "设计师",
        ...
        self.assertEqual("赵敏", deco["姓名"])
        self.assertEqual("金螳螂装饰", deco["公司"])
        self.assertEqual("设计师", deco["职位"])
        self.assertNotEqual("金螳螂装", deco["姓名"])

        deco_title = ocr_engine._map_business_card_fields(
            [
                "金螳螂装饰",
                "装饰设计师",
                "赵敏",
        ...
        self.assertEqual("赵敏", deco_title["姓名"])
        self.assertEqual("金螳螂装饰", deco_title["公司"])
        self.assertEqual("装饰设计师", deco_title["职位"])
        self.assertNotEqual("装饰", deco_title["姓名"])

        garden = ocr_engine._map_business_card_fields(
            [
                "东方园林",
                "刘洋",
                "工程师",
        ...
        self.assertEqual("刘洋", garden["姓名"])
        self.assertEqual("东方园林", garden["公司"])
        self.assertEqual("北京市朝阳区", garden["地址"])
        self.assertNotEqual("东方园林", garden["姓名"])
        self.assertNotEqual("东方园林", garden["地址"])

        garden_addr = ocr_engine._map_business_card_fields(
            [
                "东方园林",
                "刘洋",
                "工程师",
        ...
        self.assertEqual("刘洋", garden_addr["姓名"])
        self.assertEqual("东方园林", garden_addr["公司"])
        self.assertEqual("园林路88号", garden_addr["地址"])
        self.assertNotEqual("园林路88号", garden_addr["公司"])

        seed = ocr_engine._map_business_card_fields(
            [
                "隆平种业",
                "孙丽",
                "研究员",
        ...
        self.assertEqual("孙丽", seed["姓名"])
        self.assertEqual("隆平种业", seed["公司"])

        farm = ocr_engine._map_business_card_fields(
            [
                "温氏农牧",
                "周杰",
                "经理",
        ...
        self.assertEqual("周杰", farm["姓名"])
        self.assertEqual("温氏农牧", farm["公司"])

        seafood = ocr_engine._map_business_card_fields(
            [
                "国联水产",
                "吴倩",
                "销售总监",
        ...
        self.assertEqual("吴倩", seafood["姓名"])
        self.assertEqual("国联水产", seafood["公司"])

        jewelry = ocr_engine._map_business_card_fields(
            [
                "周大福珠宝",
                "马超",
                "店长",
        ...
        self.assertEqual("马超", jewelry["姓名"])
        self.assertEqual("周大福珠宝", jewelry["公司"])
        self.assertEqual("店长", jewelry["职位"])
        self.assertNotEqual("周大福珠", jewelry["姓名"])

        cable = ocr_engine._map_business_card_fields(
            [
                "远东电缆",
                "黄磊",
                "工程师",
        ...
        self.assertEqual("黄磊", cable["姓名"])
        self.assertEqual("远东电缆", cable["公司"])

        valve = ocr_engine._map_business_card_fields(
            [
                "纽威阀门",
                "高峰",
                "销售经理",
        ...
        self.assertEqual("高峰", valve["姓名"])
        self.assertEqual("纽威阀门", valve["公司"])

        bearing = ocr_engine._map_business_card_fields(
            [
                "人本轴承",
                "何静",
                "工程师",
        ...
        self.assertEqual("何静", bearing["姓名"])
        self.assertEqual("人本轴承", bearing["公司"])

        tire = ocr_engine._map_business_card_fields(
            [
                "玲珑轮胎",
                "唐雪",
                "区域经理",
        ...
        self.assertEqual("唐雪", tire["姓名"])
        self.assertEqual("玲珑轮胎", tire["公司"])
        ...
        self.assertEqual("李强", furniture["姓名"])
        self.assertEqual("顾家家具", furniture["公司"])
        self.assertEqual("设计师", furniture["职位"])
        self.assertNotEqual("顾家家具", furniture["姓名"])

        furniture_addr = ocr_engine._map_business_card_fields(
            [
                "顾家家具",
                "李强",
                "设计师",
        ...
        self.assertEqual("李强", furniture_addr["姓名"])
        self.assertEqual("顾家家具", furniture_addr["公司"])
        self.assertEqual("家具路88号", furniture_addr["地址"])
        self.assertNotEqual("家具路88号", furniture_addr["公司"])

        bath = ocr_engine._map_business_card_fields(
            [
                "九牧卫浴",
                "冯伟",
                "销售总监",
        ...
        self.assertEqual("冯伟", bath["姓名"])
        self.assertEqual("九牧卫浴", bath["公司"])

        elevator = ocr_engine._map_business_card_fields(
            [
                "康力电梯",
                "曹阳",
                "工程师",
        ...
        self.assertEqual("曹阳", elevator["姓名"])
        self.assertEqual("康力电梯", elevator["公司"])

        lighting = ocr_engine._map_business_card_fields(
            [
                "欧普灯具",
                "许飞",
                "设计师",
        ...
        self.assertEqual("许飞", lighting["姓名"])
        self.assertEqual("欧普灯具", lighting["公司"])

        packing = ocr_engine._map_business_card_fields(
            [
                "裕同包装",
                "郑华",
                "经理",
        ...
        self.assertEqual("郑华", packing["姓名"])
        self.assertEqual("裕同包装", packing["公司"])

        bookstore = ocr_engine._map_business_card_fields(
            [
                "西西弗书店",
                "林涛",
                "店长",
        ...
        self.assertEqual("林涛", bookstore["姓名"])
        self.assertEqual("西西弗书店", bookstore["公司"])
        self.assertNotEqual("西西弗书", bookstore["姓名"])

        cinema = ocr_engine._map_business_card_fields(
            [
                "万达影院",
                "刘洋",
                "经理",
        ...
        self.assertEqual("刘洋", cinema["姓名"])
        self.assertEqual("万达影院", cinema["公司"])

        hydro = ocr_engine._map_business_card_fields(
            [
                "长江水电",
                "李强",
                "工程师",
        ...
        self.assertEqual("李强", hydro["姓名"])
        self.assertEqual("长江水电", hydro["公司"])

        wind = ocr_engine._map_business_card_fields(
            [
                "金风风电",
                "陈明",
                "工程师",
        ...
        self.assertEqual("陈明", wind["姓名"])
        self.assertEqual("金风风电", wind["公司"])

        tea = ocr_engine._map_business_card_fields(
            [
                "天福茶业",
                "赵敏",
                "店长",
        ...
        self.assertEqual("赵敏", tea["姓名"])
        self.assertEqual("天福茶业", tea["公司"])

        feed = ocr_engine._map_business_card_fields(
            [
                "海大饲料",
                "孙丽",
                "经理",
        ...
        self.assertEqual("孙丽", feed["姓名"])
        self.assertEqual("海大饲料", feed["公司"])

        dental_cn = ocr_engine._map_business_card_fields(
            [
                "瑞尔口腔",
                "马超",
                "医师",
        ...
        self.assertEqual("马超", dental_cn["姓名"])
        self.assertEqual("瑞尔口腔", dental_cn["公司"])
        self.assertEqual("医师", dental_cn["职位"])

        eye = ocr_engine._map_business_card_fields(
            [
                "爱尔眼科",
                "黄磊",
                "主任",
        ...
        self.assertEqual("黄磊", eye["姓名"])
        self.assertEqual("爱尔眼科", eye["公司"])

        checkup = ocr_engine._map_business_card_fields(
            [
                "美年体检",
                "高峰",
                "经理",
        ...
        self.assertEqual("高峰", checkup["姓名"])
        self.assertEqual("美年体检", checkup["公司"])

        coffee = ocr_engine._map_business_card_fields(
            [
                "瑞幸咖啡",
                "何静",
                "店长",
        ...
        self.assertEqual("何静", coffee["姓名"])
        self.assertEqual("瑞幸咖啡", coffee["公司"])
        self.assertEqual("店长", coffee["职位"])

        beauty = ocr_engine._map_business_card_fields(
            [
                "自然美美容",
                "唐雪",
                "顾问",
        ...
        self.assertEqual("唐雪", beauty["姓名"])
        self.assertEqual("自然美美容", beauty["公司"])
        self.assertNotEqual("自然美美", beauty["姓名"])
        ...
        self.assertEqual("冯伟", security["姓名"])
        self.assertEqual("大华安防", security["公司"])

        eco = ocr_engine._map_business_card_fields(
            [
                "首创环保",
                "曹阳",
                "经理",
        ...
        self.assertEqual("曹阳", eco["姓名"])
        self.assertEqual("首创环保", eco["公司"])
        self.assertNotEqual("首创环保", eco["姓名"])

        eco_title = ocr_engine._map_business_card_fields(
            [
                "首创环保",
                "环保经理",
                "曹阳",
        ...
        self.assertEqual("曹阳", eco_title["姓名"])
        self.assertEqual("首创环保", eco_title["公司"])
        self.assertEqual("环保经理", eco_title["职位"])
        self.assertNotEqual("环保", eco_title["姓名"])

        heat = ocr_engine._map_business_card_fields(
            [
                "北京热力",
                "许飞",
                "工程师",
        ...
        self.assertEqual("许飞", heat["姓名"])
        self.assertEqual("北京热力", heat["公司"])

        space = ocr_engine._map_business_card_fields(
            [
                "中国航天",
                "张伟",
                "工程师",
        ...
        self.assertEqual("张伟", space["姓名"])
        self.assertEqual("中国航天", space["公司"])

        nuclear = ocr_engine._map_business_card_fields(
            [
                "中国核电",
                "王芳",
                "工程师",
        ...
        self.assertEqual("王芳", nuclear["姓名"])
        self.assertEqual("中国核电", nuclear["公司"])

        credit_union = ocr_engine._map_business_card_fields(
            [
                "农村信用社",
                "李强",
                "客户经理",
        ...
        self.assertEqual("李强", credit_union["姓名"])
        self.assertEqual("农村信用社", credit_union["公司"])
        self.assertNotEqual("农村信用", credit_union["姓名"])

        automotive = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Automotive",
                "Director",
        ...
        self.assertEqual("Jane Doe", automotive["姓名"])
        self.assertEqual("Acme Automotive", automotive["公司"])
        self.assertEqual("Director", automotive["职位"])
        self.assertNotEqual("Acme Automotive", automotive["姓名"])

        auto_title = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Automotive Engineer",
                "Jane Doe",
        ...
        self.assertEqual("Jane Doe", auto_title["姓名"])
        self.assertEqual("Acme Inc", auto_title["公司"])
        self.assertEqual("Automotive Engineer", auto_title["职位"])
        self.assertNotEqual("Automotive", auto_title["姓名"])

        pictures = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Paramount Pictures",
                "Director",
        ...
        self.assertEqual("Jane Doe", pictures["姓名"])
        self.assertEqual("Paramount Pictures", pictures["公司"])
        self.assertNotEqual("Paramount Pictures", pictures["姓名"])

        furniture_en = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Furniture",
                "Designer",
        ...
        self.assertEqual("Jane Doe", furniture_en["姓名"])
        self.assertEqual("Acme Furniture", furniture_en["公司"])
        self.assertEqual("Designer", furniture_en["职位"])
        ...
        self.assertEqual("Jane Doe", clinic["姓名"])
        self.assertEqual("Mayo Clinic", clinic["公司"])
        self.assertNotEqual("Mayo Clinic", clinic["姓名"])

        clinic_title = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Clinic Manager",
                "Jane Doe",
        ...
        self.assertEqual("Jane Doe", clinic_title["姓名"])
        self.assertEqual("Acme Inc", clinic_title["公司"])
        self.assertEqual("Clinic Manager", clinic_title["职位"])

        aerospace = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Aerospace",
                "Director",
        ...
        self.assertEqual("Jane Doe", aerospace["姓名"])
        self.assertEqual("Acme Aerospace", aerospace["公司"])

        dynamics = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Dynamics",
                "Director",
        ...
        self.assertEqual("Jane Doe", dynamics["姓名"])
        self.assertEqual("Acme Dynamics", dynamics["公司"])

        services = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Services",
                "Director",
        ...
        self.assertEqual("Jane Doe", services["姓名"])
        self.assertEqual("Acme Services", services["公司"])
        ...
        self.assertEqual("Jane Doe", trust["姓名"])
        self.assertEqual("Northern Trust", trust["公司"])

        trust_title = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Trust Officer",
                "Jane Doe",
        ...
        self.assertEqual("Jane Doe", trust_title["姓名"])
        self.assertEqual("Acme Inc", trust_title["公司"])
        self.assertEqual("Trust Officer", trust_title["职位"])
        ...
        self.assertEqual("Jane Doe", solar["姓名"])
        self.assertEqual("Acme Solar", solar["公司"])

        aviation = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Aviation",
                "Director",
        ...
        self.assertEqual("Jane Doe", aviation["姓名"])
        self.assertEqual("Acme Aviation", aviation["公司"])

        marine = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Marine",
                "Director",
        ...
        self.assertEqual("Jane Doe", marine["姓名"])
        self.assertEqual("Acme Marine", marine["公司"])

        publishing = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Publishing",
                "Director",
        ...
        self.assertEqual("Jane Doe", publishing["姓名"])
        self.assertEqual("Acme Publishing", publishing["公司"])

        cosmetics = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Cosmetics",
                "Director",
        ...
        self.assertEqual("Jane Doe", cosmetics["姓名"])
        self.assertEqual("Acme Cosmetics", cosmetics["公司"])
        ...
        self.assertEqual("Jane Doe", pharmacy["姓名"])
        self.assertEqual("Acme Pharmacy", pharmacy["公司"])
        ...
        self.assertEqual("Jane Doe", cement["姓名"])
        self.assertEqual("Acme Cement", cement["公司"])

        entertainment = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Entertainment",
                "Director",
        ...
        self.assertEqual("Jane Doe", entertainment["姓名"])
        self.assertEqual("Acme Entertainment", entertainment["公司"])

        dental_en = ocr_engine._map_business_card_fields(
            [
                "Jane Doe",
                "Acme Dental",
                "Director",
        ...
        self.assertEqual("Jane Doe", dental_en["姓名"])
        self.assertEqual("Acme Dental", dental_en["公司"])
        self.assertNotEqual("Acme Dental", dental_en["姓名"])

    def test_film_director_electrician_intern_are_titles_not_names(self):
        director = ocr_engine._map_business_card_fields(
            [
                "光线传媒",
                "导演",
        ...
        self.assertEqual("张艺", director["姓名"])
        self.assertEqual("光线传媒", director["公司"])
        self.assertEqual("导演", director["职位"])
        self.assertNotEqual("导演", director["姓名"])

        mixed_director = ocr_engine._map_business_card_fields(
            [
                "导演 张艺",
                "光线传媒",
                "13800138000",
        ...
        self.assertEqual("张艺", mixed_director["姓名"])
        self.assertEqual("导演 张艺", mixed_director["职位"])
        self.assertEqual("光线传媒", mixed_director["公司"])

        electrician = ocr_engine._map_business_card_fields(
            [
                "国家电网",
                "电工",
                "张伟",
        ...
        self.assertEqual("张伟", electrician["姓名"])
        self.assertEqual("国家电网", electrician["公司"])
        self.assertEqual("电工", electrician["职位"])
        self.assertNotEqual("电工", electrician["姓名"])

        mixed_electrician = ocr_engine._map_business_card_fields(
            [
                "电工 张伟",
                "国家电网",
                "13800138001",
        ...
        self.assertEqual("张伟", mixed_electrician["姓名"])
        self.assertEqual("电工 张伟", mixed_electrician["职位"])

        welder = ocr_engine._map_business_card_fields(
            [
                "中国船舶",
                "焊工",
                "李强",
        ...
        self.assertEqual("李强", welder["姓名"])
        self.assertEqual("中国船舶", welder["公司"])
        self.assertEqual("焊工", welder["职位"])
        self.assertNotEqual("焊工", welder["姓名"])

        planner = ocr_engine._map_business_card_fields(
            [
                "光线传媒",
                "策划",
                "王芳",
        ...
        self.assertEqual("王芳", planner["姓名"])
        self.assertEqual("策划", planner["职位"])
        self.assertNotEqual("策划", planner["姓名"])

        artist = ocr_engine._map_business_card_fields(
            [
                "光线传媒",
                "美工",
                "陈明",
        ...
        self.assertEqual("陈明", artist["姓名"])
        self.assertEqual("美工", artist["职位"])
        self.assertNotEqual("美工", artist["姓名"])

        ops = ocr_engine._map_business_card_fields(
            [
                "杭州云启科技",
                "运维",
                "赵敏",
        ...
        self.assertEqual("赵敏", ops["姓名"])
        self.assertEqual("运维", ops["职位"])
        self.assertNotEqual("运维", ops["姓名"])

        broker = ocr_engine._map_business_card_fields(
            [
                "链家地产",
                "经纪人",
                "刘洋",
        ...
        self.assertEqual("刘洋", broker["姓名"])
        self.assertEqual("链家地产", broker["公司"])
        self.assertEqual("经纪人", broker["职位"])
        self.assertNotEqual("经纪人", broker["姓名"])

        receptionist = ocr_engine._map_business_card_fields(
            [
                "万科物业",
                "前台",
                "李娜",
        ...
        self.assertEqual("李娜", receptionist["姓名"])
        self.assertEqual("万科物业", receptionist["公司"])
        self.assertEqual("前台", receptionist["职位"])
        self.assertNotEqual("前台", receptionist["姓名"])

        waiter = ocr_engine._map_business_card_fields(
            [
                "海底捞餐饮",
                "服务员",
                "孙丽",
        ...
        self.assertEqual("孙丽", waiter["姓名"])
        self.assertEqual("服务员", waiter["职位"])
        self.assertNotEqual("服务员", waiter["姓名"])

        intern = ocr_engine._map_business_card_fields(
            [
                "阿里巴巴集团",
                "实习生",
                "周杰",
        ...
        self.assertEqual("周杰", intern["姓名"])
        self.assertEqual("实习生", intern["职位"])
        self.assertNotEqual("实习生", intern["姓名"])

        barista = ocr_engine._map_business_card_fields(
            [
                "瑞幸咖啡",
                "咖啡师",
                "何静",
        ...
        self.assertEqual("何静", barista["姓名"])
        self.assertEqual("瑞幸咖啡", barista["公司"])
        self.assertEqual("咖啡师", barista["职位"])
        self.assertNotEqual("咖啡师", barista["公司"])

        technician = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Technician Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", technician["姓名"])
        self.assertEqual("Technician Jane Doe", technician["职位"])
        self.assertEqual("Acme Inc", technician["公司"])
        self.assertNotEqual("Technician Jane Doe", technician["姓名"])

        driver_en = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Driver",
                "John Smith",
        ...
        self.assertEqual("John Smith", driver_en["姓名"])
        self.assertEqual("Driver", driver_en["职位"])
        self.assertNotEqual("Driver", driver_en["姓名"])

        intern_en = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Intern Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", intern_en["姓名"])
        self.assertEqual("Intern Jane Doe", intern_en["职位"])
        self.assertNotEqual("Intern Jane Doe", intern_en["姓名"])

        dentist = ocr_engine._map_business_card_fields(
            [
                "Acme Dental",
                "Dentist Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", dentist["姓名"])
        self.assertEqual("Acme Dental", dentist["公司"])
        self.assertEqual("Dentist Jane Doe", dentist["职位"])
        self.assertNotEqual("Acme Dental", dentist["姓名"])

        pilot = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Pilot Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", pilot["姓名"])
        self.assertEqual("Pilot Jane Doe", pilot["职位"])

        editor = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Editor Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", editor["姓名"])
        self.assertEqual("Editor Jane Doe", editor["职位"])

        producer = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Producer Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", producer["姓名"])
        self.assertEqual("Producer Jane Doe", producer["职位"])

        receptionist_en = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
        ...
        self.assertEqual("Jane Doe", receptionist_en["姓名"])
        self.assertEqual("Receptionist Jane Doe", receptionist_en["职位"])

        mechanic = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Mechanic John Smith",
                "1 Main St",
        ...
        self.assertEqual("John Smith", mechanic["姓名"])
        self.assertEqual("Mechanic John Smith", mechanic["职位"])

        electrician_en = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Electrician Jane Doe",
                "1 Main St",
            ]
        )
        self.assertEqual("Jane Doe", electrician_en["姓名"])
        self.assertEqual("Electrician Jane Doe", electrician_en["职位"])
        ...
        self.assertEqual("Jane Doe", actuary["姓名"])
        self.assertEqual("Actuary Jane Doe", actuary["职位"])

        broker_en = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Broker Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", broker_en["姓名"])
        self.assertEqual("Broker Jane Doe", broker_en["职位"])

        stylist = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Stylist Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", stylist["姓名"])
        self.assertEqual("Stylist Jane Doe", stylist["职位"])

        inspector = ocr_engine._map_business_card_fields(
            [
                "Acme Inc",
                "Inspector Jane Doe",
                "1 Main St",
        ...
        self.assertEqual("Jane Doe", inspector["姓名"])
        self.assertEqual("Inspector Jane Doe", inspector["职位"])

    def test_field_labels_are_not_used_as_name_or_company(self):
        colon = ocr_engine._map_business_card_fields(
            [
                "姓名：张三",
                "杭州云启科技有限公司",
                "销售经理",
        ...
        self.assertEqual("张三", colon["姓名"])
        self.assertEqual("杭州云启科技有限公司", colon["公司"])
        self.assertEqual("销售经理", colon["职位"])
        self.assertEqual("杭州市西湖区文三路100号", colon["地址"])
        self.assertEqual("13800138000", colon["手机"])

        split_labels = ocr_engine._map_business_card_fields(
            [
                "姓名",
                "张三",
                "公司",
        ...
        self.assertEqual("张三", split_labels["姓名"])
        self.assertEqual("杭州云启科技有限公司", split_labels["公司"])
        self.assertEqual("销售经理", split_labels["职位"])
        self.assertEqual("杭州市西湖区文三路100号", split_labels["地址"])

        given_name = ocr_engine._map_business_card_fields(
            ["名字：李四", "某某集团", "总监"]
        )
        self.assertEqual("李四", given_name["姓名"])
        self.assertEqual("某某集团", given_name["公司"])

        contact = ocr_engine._map_business_card_fields(
            ["联系人：王五", "某某公司", "经理"]
        )
        self.assertEqual("王五", contact["姓名"])
        self.assertEqual("某某公司", contact["公司"])

        workplace = ocr_engine._map_business_card_fields(
            ["工作单位", "杭州云启科技", "张三", "经理"]
        )
        self.assertEqual("张三", workplace["姓名"])
        self.assertEqual("杭州云启科技", workplace["公司"])
        self.assertNotEqual("工作单位", workplace["姓名"])

        unit = ocr_engine._map_business_card_fields(
            ["单位：字节跳动", "产品经理", "赵六"]
        )
        self.assertEqual("赵六", unit["姓名"])
        self.assertEqual("字节跳动", unit["公司"])
        self.assertEqual("产品经理", unit["职位"])

        concatenated = ocr_engine._map_business_card_fields(
            ["姓名张三", "杭州云启科技有限公司", "经理"]
        )
        self.assertEqual("张三", concatenated["姓名"])

        unlabeled = ocr_engine._map_business_card_fields(
            [
                "张三",
                "销售经理",
                "杭州云启科技有限公司",
        ...
        self.assertEqual("张三", unlabeled["姓名"])
        self.assertEqual("杭州云启科技有限公司", unlabeled["公司"])

        english = ocr_engine._map_business_card_fields(
            ["Name: Jane Doe", "Acme Inc", "Director", "1 Main St"]
        )
        self.assertEqual("Jane Doe", english["姓名"])
        self.assertEqual("Acme Inc", english["公司"])
        self.assertEqual("Director", english["职位"])

        company_director = ocr_engine._map_business_card_fields(
            ["Jane Doe", "Company Director", "Acme Inc"]
        )
        self.assertEqual("Jane Doe", company_director["姓名"])
        self.assertEqual("Acme Inc", company_director["公司"])
        self.assertEqual("Company Director", company_director["职位"])
