...
def handler(event, context):
    ...
    try:
        ...
        if _all_fields_empty(cleaned_result):
            return _response(422, {"success": False, "error": "未识别到有效字段，请确认图片和模板是否匹配"})
...
def _all_fields_empty(result: dict) -> bool:
    return all(not str(value).strip() for value in result.values())