...
def _extract_name(chunks: list[str]) -> str:
    ...
    # 路/街/号/大厦/楼只跳过非二字中文名，避免“张路”被当成地址行丢掉。
    cn_address_markers = ("路", "街", "号", "大厦", "楼", "巷", "弄")
...
_CN_ADDRESS_KEYWORDS = ("地址", "路", "街", "号", "区", "市", "省", "大厦", "楼", "巷", "弄")
_EN_ADDRESS_WORDS = (
    "add",
    "address",
    "road",
...
def _looks_like_valid_cn_name(value: str) -> bool:
    ...
    if any(value.startswith(prefix) for prefix in non_name_prefixes):
        return False
    # 3 字及以上含路/街/号/大厦/楼更像地址（中山路/科技大厦），二字名（张路）仍保留
    if len(value) >= 3 and any(marker in value for marker in ("路", "街", "号", "大厦", "楼")):
        return False
    return True
...
_STRONG_ADDRESS_MARKERS = ("地址", "Address", "路", "街", "道", "大厦")
