...
# 名片座机常写成 (021) 1234-5678 / 021.12345678，分隔符还要覆盖括号和点号。
_PHONE_SEP = r"[\s\-－—–.()（）]"
...
def _extract_name(chunks: list[str]) -> str:
    ...
    # 路/街/号/道/大厦/楼/巷/弄/座/栋/层/室/园/广场只跳过非二字中文名，避免“张路”“李园”被当成地址行丢掉。
    cn_address_markers = ("路", "街", "号", "道", "大厦", "楼", "巷", "弄", "座", "栋", "层", "室", "园", "广场")
...
def _looks_like_valid_cn_name(value: str) -> bool:
    ...
    if len(value) >= 3 and any(
        marker in value
        for marker in ("路", "街", "号", "道", "大厦", "楼", "座", "栋", "巷", "弄", "层", "室", "园", "广场")
    ):
        return False
    return True
...
_STRONG_ADDRESS_MARKERS = ("地址", "Address", "路", "街", "道", "大厦", "座", "栋", "巷", "弄", "园", "广场")
