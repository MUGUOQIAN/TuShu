...
def _call_glm(
    ...
    # GLM-OCR 要求 file 使用 URL 或 data URI；必要时压缩并保留正确MIME。
    ...
    # 1) 仅当候选对象真实包含业务字段时才直接返回；layout外壳不能当结果。
    for candidate in (data.get("data"), data.get("result"), data):
        if isinstance(candidate, dict) and _has_expected_fields(candidate, expected_fields):
            return candidate
...
def _prepare_file_data_uri(
    ...
    """生成 GLM 可消费的 data URI，只有超限时转 JPEG 压缩。"""
    ...
    with Image.open(io.BytesIO(image_bytes)) as img:
        ...
        if max(width, height) <= max_edge and len(image_bytes) <= max_bytes:
            mime = _mime_for_image_format(original_format)
...
def _mime_for_image_format(image_format: str | None) -> str:
...
def _has_expected_fields(candidate: dict, expected_fields: list[str] | None) -> bool:
    if not expected_fields:
        return True
    return any(field in candidate for field in expected_fields)


def _structure_text_with_glm(text: str, prompt: str) -> dict:
    """将 layout OCR 文本二次结构化为业务字段JSON。"""
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "你是严格的信息抽取器，只输出JSON对象。"},
...
def _extract_text_chunks(data: dict) -> list[str]:
    ...
    def walk_container(value):
        ...
        if isinstance(md_results, list):
            ...
            append_text(md_results)

        if "layout_details" in value:
            walk_layout(value.get("layout_details"))

        for key in ("data", "result"):
            nested = value.get(key)
            if isinstance(nested, dict):
                walk_container(nested)
...
def _extract_name(chunks: list[str]) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if en:
            ...
            looks_like_company = _contains_english_company_keyword(en_low, company_en_keywords)
...
def _contains_english_company_keyword(value: str, keywords: tuple[str, ...]) -> bool:
