...
def _call_glm(
    ...
    # 1) 只有真正包含目标业务字段时才把响应当作结构化结果；
    # layout_parsing 的 data/result 外壳通常只包含 md_results/layout_details。
    structured = _find_structured_payload(data, expected_fields)
    if structured is not None:
        return structured

    # 2) 从 layout_parsing 结果中提取文本。名片保留本地映射，发票/自定义
    # 需要用 OCR 文本再按用户选择的模板结构化。
...
def _find_structured_payload(data: dict, expected_fields: list[str] | None) -> dict | None:
    if _contains_expected_fields(data, expected_fields):
        return data

    for key in ("data", "result"):
        value = data.get(key)
        if isinstance(value, dict) and _contains_expected_fields(value, expected_fields):
            return value
    return None


def _contains_expected_fields(data: dict, expected_fields: list[str] | None) -> bool:
...
def _extract_structured_fields(
    text_chunks: list[str], prompt: str, expected_fields: list[str] | None
) -> dict:
    ...
    user_prompt = (
        f"{prompt.strip()}\n\n"
        "以下是 OCR 识别出的原始文本，请只基于这些文本抽取字段，返回 JSON 对象：\n"
        f"{ocr_text}"
    )
    payload = {
        "model": TEXT_MODEL,
...
def _extract_chat_content(data: dict) -> str:
    ...
    if not isinstance(content, str) or not content.strip():
        raise ValueError("GLM结构化响应缺少message.content")
    return content
...
def _extract_text_chunks(data: dict) -> list[str]:
    ...
    def collect(value):
        ...
        for key in ("md_results", "layout_details"):
            if key in value:
                collect(value[key])
