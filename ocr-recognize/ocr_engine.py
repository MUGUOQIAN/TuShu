...
def _call_glm(
    ...
    # 1) 仅当返回对象含业务字段时直接使用，避免把 layout 外壳误当结果。
    for candidate in (data, data.get("data"), data.get("result")):
        if isinstance(candidate, dict) and _contains_expected_fields(
            candidate, expected_fields
        ):
            return candidate
...
def _contains_expected_fields(
    candidate: dict, expected_fields: list[str] | None
) -> bool:
    return bool(expected_fields) and any(
        field in candidate for field in expected_fields
    )
...
def _prepare_file_data_uri(
    ...
    raise ValueError("无法处理图片数据")


def _compress_base64_image(
    image_base64: str, max_edge: int = 1280, max_bytes: int = 450 * 1024
) -> str:
    """兼容旧调用方：返回规范化 data URI 中的 Base64 部分。"""
    data_uri = _prepare_file_data_uri(
...
def _extract_structured_fields(
    ...
    """将 layout OCR 文本按发票/自定义模板结构化为业务字段。"""
    ...
    if resp.status_code >= 400:
        raise RuntimeError(
            f"GLM结构化请求失败 status={resp.status_code}, body={resp.text[:500]}"
        )
    data = resp.json()
