...
def _call_glm(
    ...
    # 只把包含目标字段的对象当成业务结果，不能把 layout API 外壳直接返回。
    direct_result = _find_expected_result(data, expected_fields or [])
    if direct_result is not None:
        return direct_result

    # layout_parsing 只负责 OCR；按模板将 OCR 文本转换为最终字段。
...
def _find_expected_result(data: dict, expected_fields: list[str]) -> dict | None:
    if expected_fields and any(field in data for field in expected_fields):
        return data
    for key in ("data", "result"):
        nested = data.get(key)
        if isinstance(nested, dict):
            result = _find_expected_result(nested, expected_fields)
...
def _extract_structured_fields(
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你是严谨的信息抽取助手，只输出包含指定字段的JSON对象。",
            },
            {
                "role": "user",
                "content": (
                    f"{prompt}\n\n"
                    f"以下是OCR文本。只根据文本抽取，未找到的字段填空字符串：\n"
                    f"{ocr_text}"
                ),
            },
        ],
...
def _extract_text_chunks(data: dict) -> list[str]:
    ...
    def collect(value):
        if isinstance(value, dict):
            append_text(value.get("md_results"))
