...
def _call_glm(
    ...
    # 1) 只有命中业务字段时，才把模型返回对象当作最终抽取结果。
    business_result = _find_business_result(data, expected_fields)
    if business_result is not None:
        return business_result

    # 2) layout_parsing 只负责 OCR/版面文本；业务抽取需要按模板二次处理。
...
def _prepare_file_data_uri(image_base64: str) -> str:
    payload, mime_hint = _split_base64_image(image_base64)
    ...
    with Image.open(io.BytesIO(image_bytes)) as img:
        ...
        mime = mime_hint or _mime_from_format(original_format)

        if len(image_bytes) <= 450 * 1024 and long_edge <= 1280 and mime:
        ...
        while quality >= 35:
            ...
            if len(result) <= 450 * 1024 or quality == 35:
...
def _split_base64_image(image_base64: str) -> tuple[str, str]:
    ...
    if match:
        ...
        mime = ""
        payload = text
    return re.sub(r"\s+", "", payload), mime
...
def _find_business_result(data: dict, expected_fields: list[str] | None) -> dict | None:
    ...
    return _find_business_result_in_value(data, expected_fields)


def _find_business_result_in_value(value: Any, expected_fields: list[str]) -> dict | None:
    ...
    for key in ("data", "result"):
        ...
        found = _find_business_result_in_value(nested, expected_fields)
...
def _structure_text_with_glm(ocr_text: str, prompt: str, expected_fields: list[str]) -> dict:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你只输出一个JSON对象，不要包含Markdown或解释文字。",
            },
            {
                "role": "user",
    ...
    business_result = _find_business_result(parsed, expected_fields)
    return business_result if business_result is not None else parsed
    ...
    def visit(value: Any):
        if isinstance(value, dict):
            ...
            if isinstance(md_results, str) and md_results.strip():
                chunks.extend(part.strip() for part in md_results.splitlines() if part.strip())
            elif isinstance(md_results, list):
                for item in md_results:
                    visit(item)

            layout_details = value.get("layout_details")
            if isinstance(layout_details, (list, dict)):
            ...
            chunks.extend(part.strip() for part in value.splitlines() if part.strip())
...
def _extract_name(chunks: list[str]) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if en:
            ...
            looks_like_company = _contains_company_keyword(en_low, company_en_keywords)
