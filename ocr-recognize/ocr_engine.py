...
# 名片 OCR 常把「姓名/公司」印成独立标签行，或写成「姓名：张三」。
# 标签本身不能当字段值，否则姓名变成「姓名」、公司变成「公司」。
# 更长的标签必须排在前面，避免「工作单位」被「单位」先吃掉。
_CN_FIELD_LABELS = (
    ("工作单位", "公司"),
    ("联系人", "姓名"),
    ("姓名", "姓名"),
...
_CN_NAME_LABEL_WORDS = ("姓名", "名字", "联系人", "公司", "单位", "工作单位", "职位", "职务", "地址", "部门")


def _parse_field_label(chunk: str) -> tuple[str | None, str]:
    text = chunk.strip()
    if not text:
        return None, ""
    for label, field in _CN_FIELD_LABELS:
        if text == label:
            return field, ""
        if text.startswith(label):
            sep = text[len(label) : len(label) + 1]
            if sep in " \t:：":
                return field, text[len(label) :].lstrip(" \t:：")
    low = text.lower()
    for label, field in _EN_FIELD_LABELS:
        if low == label:
            return field, ""
        if low.startswith(label):
            sep = text[len(label) : len(label) + 1]
            # 英文只认冒号，避免把 Company Director 当成公司标签。
            if sep in ":：":
                return field, text[len(label) :].lstrip(" \t:：")
    return None, text


def _strip_name_field_prefix(value: str) -> str:
    for label in ("姓名", "名字", "联系人"):
        if value.startswith(label) and len(value) > len(label):
            rest = value[len(label) :]
            if _looks_like_valid_cn_name(rest):
                return rest
    return value
...
def _map_business_card_fields(chunks: list[str]) -> dict:
    ...
    mapping_chunks: list[str] = []
    name_chunks: list[str] = []
    for chunk in chunks:
        if not chunk.strip():
            continue
        field, rest = _parse_field_label(chunk)
        if field is not None:
            if rest:
                labeled.setdefault(field, rest)
                mapping_chunks.append(rest)
                if field == "姓名":
                    name_chunks.append(rest)
            continue
    ...
    company = labeled.get("公司") or (company_chunks[0] if company_chunks else "")
    title = labeled.get("职位") or (title_chunks[0] if title_chunks else "")
    ...
    if dedicated_strong:
        ...
        address = labeled.get("地址") or ""

    name = labeled.get("姓名") or _extract_name(name_chunks)
...
def _extract_name(chunks: list[str]) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if cn:
            cn_name = _strip_name_field_prefix(cn.group(0))
...
# 工程/机械/电器/乳业/体育 同理：海油工程、徐工机械、格力电器、伊利乳业、安踏体育是公司，
# 工程经理、机械工程师、电器工程师、体育老师、工程路（含路）只是职位或地址。
# 煤业/水务/燃气/装饰/家具/电缆 同理：兖州煤业、北控水务、新奥燃气、金螳螂装饰、顾家家具是公司，
# 燃气工程师、装饰设计师、环保经理、家具路（含路）只是职位或地址。
...
# Engineering/Machinery/Industries 会出现在 Acme Engineering / Acme Machinery / Acme Industries，
# 也会出现在 Engineering Manager / Machinery Manager / Industries Director。
# Automotive/Aerospace/Furniture/Clinic 会出现在 Acme Automotive / Mayo Clinic / Acme Furniture，
# 也会出现在 Automotive Engineer / Clinic Manager / Furniture Designer。
...
def _is_company_line(chunk: str) -> bool:
    ...
    # “东方园林”的“园”来自公司后缀本身，去掉弱公司词后若不再有路/街/号等信号，仍应当公司。
    has_weak = any(k in chunk for k in _CN_WEAK_COMPANY_KEYWORDS) or _has_whole_word(
        low, _EN_WEAK_COMPANY_WORDS
    )
    if has_weak and not _is_title_line(chunk):
        if not _has_strong_address_signal(chunk):
            return True
        remainder = chunk
        for keyword in sorted(_CN_WEAK_COMPANY_KEYWORDS, key=len, reverse=True):
            remainder = remainder.replace(keyword, " ")
        remainder = re.sub(r"\s+", " ", remainder).strip()
        if remainder and not _has_strong_address_signal(remainder):
            return True
    return False
...
def _looks_like_valid_cn_name(value: str) -> bool:
    ...
    if value in _CN_COMPANY_KEYWORDS or value in _CN_WEAK_COMPANY_KEYWORDS:
        return False
    if value in _CN_NAME_LABEL_WORDS:
        return False
