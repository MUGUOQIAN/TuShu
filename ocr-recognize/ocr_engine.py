...
def _call_glm(
    ...
    # GLM-OCR 要求 file 使用 URL 或 data URI；大图压缩为 JPEG，小图保留真实 MIME。
    ...
    for candidate in candidates:
        text_chunks.extend(_extract_text_chunks(candidate))
    text_chunks = _dedupe(text_chunks)

    if template_type == "business_card":
        return _map_business_card_fields(text_chunks)
    return _structure_fields_from_text(text_chunks, prompt, expected_fields or [])
...
def _prepare_file_data_uri(
    ...
    with Image.open(io.BytesIO(image_bytes)) as img:
        ...
        if (
            ...
            and original_format in mime_by_format
        ):
            return f"data:{mime_by_format[original_format]};base64,{image_base64}"
...
def _compress_base64_image(
    ...
    """兼容旧测试/调用方：返回准备上传后的 base64 内容，不含 data URI 前缀。"""
    ...
    def visit(node) -> None:
        if isinstance(node, dict):
            for key, value in node.items():
                if key in {"md_results", "content"}:
                    if isinstance(value, str):
                        add_text(value)
                    elif isinstance(value, list):
                        visit(value)
                elif key in {"layout_details", "data", "result"}:
                    visit(value)
...
def _structure_fields_from_text(
    ...
    user_prompt = (
        f"{prompt.strip()}\n\n"
        "以下是 OCR layout_parsing 识别出的文本。请只根据这些文本抽取字段，"
        "不要臆测，缺失字段用空字符串。\n\n"
        f"{text}"
    )
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "你是严格的 OCR 结构化抽取器，只输出 JSON 对象。"},
