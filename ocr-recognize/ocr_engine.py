...
def _call_glm(
    ...
    # 1) 仅当返回体确实包含业务字段时才直接使用，避免把 layout 外壳当结果。
    business_result = _find_expected_fields_dict(data, expected_fields)
    if business_result is not None:
        return business_result

    # 2) 从 layout_parsing 结果中提取文本；layout API 不会消费业务抽取 prompt。
    text_chunks = _extract_text_chunks(data)
    if template_type == "business_card":
        return _map_business_card_fields(text_chunks)

    # 3) 发票/自定义字段必须在 OCR 文本基础上再做一次结构化抽取。
...
def _find_expected_fields_dict(value: dict, expected_fields: list[str] | None) -> dict | None:
    if not expected_fields or not isinstance(value, dict):
        return None
    ...
    for key in ("data", "result"):
        ...
        if isinstance(nested, dict):
            found = _find_expected_fields_dict(nested, expected_fields)
...
def _structure_text_with_glm(text_chunks: list[str], prompt: str, expected_fields: list[str]) -> dict:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你是严格的OCR结构化抽取器。只能返回JSON对象，不要输出解释文字。"
            },
            {
                "role": "user",
                "content": (
    ...
    business_result = _find_expected_fields_dict(parsed, expected_fields)
    if business_result is not None:
        return business_result

    if expected_fields:
        raise ValueError("GLM结构化返回缺少期望字段")
    return parsed
    ...
    def visit(value):
        ...
        if isinstance(value, dict):
            for key, item in value.items():
                if key in ("md_results", "layout_details", "data", "result", "content"):
                    visit(item)
